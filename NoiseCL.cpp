...
void NoiseCL::CreateImage(int width, int height)
{
	...
	std::string kernel_code =
		...
		"	}																									\n"
		"																										\n"
		"   void kernel simple_add(global float* data)															\n"
		"	{																									\n"
		...
		"       data[idx] = 0.5f + 0.5f * PerlinNoise(uv);														\n"
		"   }																									\n";
	...
	cl::Buffer buffer_data(context, CL_MEM_READ_WRITE, buffer_size);
	...
	simple_add.setArg(0, buffer_data);

	// Dispatch with workgroup parameter 
	queue.enqueueNDRangeKernel(simple_add, cl::NullRange, cl::NDRange(width, height), cl::NDRange(32,32));
